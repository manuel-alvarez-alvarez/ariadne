...
2. The planner discusses the breakdown with you, creates tasks through the
   ...
   `ariadne goal approve` when the plan is right. What each agent runs on is
   yours to pick, not the planner's: its tasks run on the agents and models
   their profiles are on until you say otherwise with `ariadne task update
   <task-id> --agent claude_code --reviewer Reviewer=codex:o3`, which a task
   takes while it is still pending or ready (`--agent default` hands it back
   to the profile's). A reviewer slot is spelled `<profile>`,
   `<profile>=<agent>` or `<profile>=<agent>:<model>`, and `ariadne task
   create` takes the same flags.
