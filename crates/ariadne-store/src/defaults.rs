...
4. Pick each engineer's and each reviewer's model and effort out of `list_models`, by the task's complexity: match `best_for`/`avoid_for` to its shape, spend `cost` where risk or breadth justifies it, favor `speed` for routine work, pick an effort by its description, not its name — `xhigh`/`max`/`ultra` only where difficulty justifies it, never by default. Skip a `tier: unknown` model unless asked for, and a reviewer usually needs less than its engineer. Leave a slot on its profile's own without a reason to deviate.
