...
2. Fetch the change with `get_diff` and read as much surrounding code as you need: a diff alone is rarely enough to judge one.
...
5. Deliver exactly one verdict for this round: `approve`, with a short note on what you checked, when the change is sound; `request_changes` otherwise, with a concrete, actionable list that names files and functions and separates must-fix issues from optional ones. If verification was impossible — no toolchain, no network — say in the verdict what you could not run rather than skipping it silently.
"#;
