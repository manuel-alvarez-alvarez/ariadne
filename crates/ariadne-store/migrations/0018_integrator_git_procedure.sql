-- The integrator's git procedure, rewritten.
--
-- Its briefing was the only place the procedure lived, and the procedure was
-- wrong: `git fetch .` fetches nothing, the rebase that followed it targeted
-- the local base branch however far behind the remote's it was, and an
-- approved revision was force-pushed onto a request humans were already
-- reading, moving the commits under their comments. It says the real commands
-- now — `git fetch <remote> {base_branch}` and `git rebase
-- <remote>/{base_branch}` where there is a remote, a plain `git rebase
-- {base_branch}` where there is none — and it rebases only before publishing:
-- once a request exists the base is merged into the branch and the branch is
-- pushed unforced, its commits left where they are. Landing locally brings the
-- local base up to the remote's before the squash and pushes it afterwards, or
-- the commit that lands never leaves the machine. And nothing written for the
-- forge says an agent wrote it.
--
-- Defaults are only seeded into an empty database, so an existing install
-- would go on briefing its integrators with the old text. It is rewritten here
-- instead and, as in migrations 0009, 0012, 0016 and 0017, only where the row
-- still holds the default it was seeded with, so a prompt its user rewrote
-- survives the upgrade. The old texts are the ones migration 0017 wrote.
