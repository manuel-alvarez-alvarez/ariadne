...
//! conversation that agent stored, and the model and the tokens of one of them
//! by its internal session id. [`DiskConversations::new`] is where a reader is
//! registered, under the registry agent id it answers for; `claude-acp` has
//! the only one today. A reader answers for its own agent and for no other.
//!
//! A reader reads a file twice and caches both reads by the file's path, size
//! and modification time: from the start of it as far as what a listing shows,
//! and the whole of it for the figures of one row. So a listing reads no file
//! it has read unchanged, and the figures of a page are read from the files of
//! that page's rows alone. The two reads stand for one stamp, so a file read
//! again for a page is read again for the listing with it.
