...
is pinned (011), what the session is briefed with (006), permission modes and
the delivery of later instructions (later tasks), and the CLI-side ACP client
...
6. Every permission request is approved: the first option the agent marks as
   allowing, the first of any kind where it marks none, and a cancelled
   outcome only where there is nothing to select. Permission modes are a
   later task.
...
- A permission request is approved with the allowing option, and the ask and
  the answer are events
  (`acp_runtime.rs::a_permission_request_is_approved_with_the_allowing_option`),
...
- Permission modes: every request is approved (rule 6).
