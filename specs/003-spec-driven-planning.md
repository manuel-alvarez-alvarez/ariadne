...
orchestrator is the one seat that talks to the user, and the one that lands a
change without owning a task.

## Scope

In: the orchestrator session, the spec conversation, the spec's folder and
format, landing the spec, writing the tasks, sizing each slot, and
`finalize_plan`.
...
1. A goal opens with one orchestrator session, started in the primary checkout
   of the goal's first repository, briefed with the goal, its repositories,
   `max_tasks`, `required_approvals` and the procedure for landing a spec.
2. The orchestrator drafts a spec from the goal — scope, behavior, acceptance
...
3. It asks the user about each unclear point: one question in plain turn text,
   ...
   (`ariadne goal attach`). An orchestrator waiting on an answer is not
   nudged and shows up wherever Ariadne lists what needs attention.
...
5. The spec file goes in the folder the repository keeps specs or docs in, and
   ...
   none, the orchestrator agrees **both a path and a format** with the user
   before writing the first one.
6. The orchestrator then lands the spec itself, with the procedure of its
   repository's merge strategy (005), and creates no task until it merges. So
   every author branches off a base that already carries the spec.
...
12. The orchestrator's work ends with the plan: once the goal is past
    `planning`, its idle session is compacted (010) and then let go.
...
- The orchestrator's own texts name no forge and no merge strategy; the
  procedure reaches it as a rendered value
  (`defaults.rs::the_orchestrator_is_told_nothing_of_forges_or_landing`,
  `prompts.rs::the_orchestrator_lands_the_spec_the_way_its_repository_takes_a_change`).
...
- Work waits on the orchestrator until it finalizes
  (`plan_finalize.rs::an_orchestrator_is_the_agent_work_waits_on_until_it_finalizes`),
  and the idle orchestrator of an active goal is ended
  (`::a_scheduler_pass_ends_the_idle_orchestrator_of_an_active_goal`).

## Sources

`crates/ariadne-store/src/defaults.rs` (`ORCHESTRATOR_SYSTEM_PROMPT`,
`ORCHESTRATOR_BRIEFING`, `ORCHESTRATOR_RESUME`, `SPEC_LANDING_*`),
`crates/ariadne-daemon/src/agents/prompts.rs`,
